...
public class Column {
    ...
    public static Column CreateColumn(String columnString){
        ...
        String name;
        if(parts.length > 2){
            Handler.UnrecognisedCommand(columnString, "Expected column format <name> <datatype> [PRIMARY KEY | NOT NULL]");
            return null;
        }
        ...
        if(parts.length > 1){
            ...
            if(type == null){
                Handler.UnrecognisedCommand(columnString, "Unrecognised data type " + parts[1]);
                return null;
            }
