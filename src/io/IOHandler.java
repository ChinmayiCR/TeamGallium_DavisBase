...
public class IOHandler
{
    ...
    {
        File databaseDir = new File(Utils.getDatabasePath(dbname));
        return  databaseDir.exists();
    }
    public boolean createTable(String databaseName, String tableName) throws ExceptionHandler
    {
        try
        {
        ...
        {
            throw new ExceptionHandler(ExceptionHandler.GENERIC_EXCEPTION);
        }
    }
    ...
    public boolean writeRecord(String databaseName, String tableName, DataRecord record) throws ExceptionHandler
    {
        RandomAccessFile randomAccessFile;
        try
        {
    ...
    private PointerRecord splitPage(RandomAccessFile randomAccessFile, Page page, DataRecord record, int pageNumber1, int pageNumber2) throws ExceptionHandler
    {
        try
        {
            if (page != null && record != null)
    ...
    private PointerRecord splitPage(RandomAccessFile randomAccessFile, Page page, DataRecord record) throws ExceptionHandler
    {
        try
        {
            if (page.getPageType() == Page.INTERIOR_TABLE_PAGE)
    ...
    private PointerRecord splitPage(RandomAccessFile randomAccessFile, Page page, PointerRecord record, int pageNumber1, int pageNumber2) throws ExceptionHandler
    {
        try
        {
            if (page != null && record != null) {
    ...
    private <T> List<T> copyRecords(RandomAccessFile randomAccessFile, long pageStartAddress, List<Short> recordAddresses, byte startIndex, byte endIndex, int pageNumber, T object) throws ExceptionHandler {
        try {
                    ...
                    for (byte j = 0; j < numberOfRecords; j++) {
                        switch (serialTypeCodes[j]) {
                            ...
                            case DatabaseConstants.ONE_BYTE_NULL_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new Text(null));
                                break;

                            case DatabaseConstants.TWO_BYTE_NULL_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new SmallInt(randomAccessFile.readShort(), true));
                                break;

                            case DatabaseConstants.FOUR_BYTE_NULL_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new Real(randomAccessFile.readFloat(), true));
                                break;
                            ...
                            case DatabaseConstants.TINY_INT_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new TinyInt(randomAccessFile.readByte()));
                                break;

                            case DatabaseConstants.SMALL_INT_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new SmallInt(randomAccessFile.readShort()));
                                break;

                            case DatabaseConstants.INT_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new Int(randomAccessFile.readInt()));
                                break;

                            case DatabaseConstants.BIG_INT_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new BigInt(randomAccessFile.readLong()));
                                break;

                            case DatabaseConstants.REAL_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new Real(randomAccessFile.readFloat()));
                                break;
                            ...
                            case DatabaseConstants.DATE_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new DateType(randomAccessFile.readLong()));
                                break;

                            case DatabaseConstants.DATE_TIME_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new DateTime(randomAccessFile.readLong()));
                                break;

                            case DatabaseConstants.TEXT_SERIAL_TYPE_CODE:
                                record.getColumnValueList().add(new Text(""));
                                break;
                            ...
                            default:
                                if (serialTypeCodes[j] > DatabaseConstants.TEXT_SERIAL_TYPE_CODE) {
                                    ...
                                    record.getColumnValueList().add(new Text(new String(text)));
                                }
                                break;
    ...
    private Page getPage(RandomAccessFile randomAccessFile, DataRecord record, int pageNumber) throws ExceptionHandler {
        try {
    ...
    private int getAddress(File file, int rowId, int pageNumber) throws ExceptionHandler {
        int location = -1;
        try {
    ...
    private int binarySearch(RandomAccessFile randomAccessFile, int key, int numberOfRecords, long seekPosition, byte pageType) throws ExceptionHandler {
        return binarySearch(randomAccessFile, key, numberOfRecords, seekPosition, pageType, false);
    }

    private int binarySearch(RandomAccessFile randomAccessFile, int key, int numberOfRecords, long seekPosition, byte pageType, boolean literalSearch) throws ExceptionHandler {
        try {
    ...
    private Page readPageHeader(RandomAccessFile randomAccessFile, int pageNumber) throws ExceptionHandler {
        try {
            Page page;
    ...
    private boolean writePageHeader(RandomAccessFile randomAccessFile, Page page) throws ExceptionHandler {
        try {
    ...
    private boolean writeRecord(RandomAccessFile randomAccessFile, DataRecord record) throws ExceptionHandler {
        try {
            ...
            for (Object object : record.getColumnValueList()) {
                switch (Utils.resolveClass(object)) {
                    case DatabaseConstants.TINYINT:
                        randomAccessFile.writeByte(((TinyInt) object).getValue());
                        break;

                    case DatabaseConstants.SMALLINT:
                        randomAccessFile.writeShort(((SmallInt) object).getValue());
                        break;

                    case DatabaseConstants.INT:
                        randomAccessFile.writeInt(((Int) object).getValue());
                        break;

                    case DatabaseConstants.BIGINT:
                        randomAccessFile.writeLong(((BigInt) object).getValue());
                        break;

                    case DatabaseConstants.REAL:
                        randomAccessFile.writeFloat(((Real) object).getValue());
                        break;
                    ...
                    case DatabaseConstants.DATE:
                        randomAccessFile.writeLong(((DateType) object).getValue());
                        break;

                    case DatabaseConstants.DATETIME:
                        randomAccessFile.writeLong(((DateTime) object).getValue());
                        break;

                    case DatabaseConstants.TEXT:
                        if (((Text) object).getValue() != null)
                            randomAccessFile.writeBytes(((Text) object).getValue());
                        break;
            ...
            throw new ExceptionHandler(ExceptionHandler.INVALID_DATATYPE_EXCEPTION);
        } catch (Exception e) {
            throw new ExceptionHandler(ExceptionHandler.GENERIC_EXCEPTION);
        }
        return true;
    ...
    private boolean writeRecord(RandomAccessFile randomAccessFile, PointerRecord record) throws ExceptionHandler {
        try {
    ...
    public List<DataRecord> findRecord(String databaseName, String tableName, InternalCondition condition, boolean getOne) throws ExceptionHandler {
        return findRecord(databaseName, tableName, condition,null, getOne);
    }

    public List<DataRecord> findRecord(String databaseName, String tableName, InternalCondition condition, List<Byte> selectionColumnIndexList, boolean getOne) throws ExceptionHandler {
    ...
    public List<DataRecord> findRecord(String databaseName, String tableName, List<InternalCondition> conditionList, boolean getOne) throws ExceptionHandler {
        return findRecord(databaseName, tableName, conditionList, null, getOne);
    }

    public List<DataRecord> findRecord(String databaseName, String tableName, List<InternalCondition> conditionList, List<Byte> selectionColumnIndexList, boolean getOne) throws ExceptionHandler {
        try {
            File file = new File(Utils.getDatabasePath(databaseName) + "/" + tableName + DatabaseConstants.DEFAULT_FILE_EXTENSION);
            if (file.exists()) {
                RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r");
    ...
    public int updateRecord(String databaseName, String tableName, InternalCondition condition, List<Byte> updateColumnIndexList, List<Object> updateColumnValueList, boolean isIncrement) throws ExceptionHandler {
    ...
    public int updateRecord(String databaseName, String tableName, List<InternalCondition> conditions, List<Byte> updateColumnIndexList, List<Object> updateColumnValueList, boolean isIncrement) throws ExceptionHandler {
                    ...
                    if (records.size() > 0) {
                        ...
                        for (DataRecord record : records) {
                            for (int i = 0; i < updateColumnIndexList.size(); i++) {
                                ...
                                if (isIncrement) {
                                    record.getColumnValueList().set(index, increment((Numeric) record.getColumnValueList().get(index), (Numeric) object));
                                } else {
    ...
    private <T> Numeric<T> increment(Numeric<T> object1, Numeric<T> object2) {
        object1.increment(object2.getValue());
        return object1;
    }

    public Page<DataRecord> getLastRecordAndPage(String databaseName, String tableName) throws ExceptionHandler {
        try {
            File file = new File(Utils.getDatabasePath(databaseName) + "/" + tableName + DatabaseConstants.DEFAULT_FILE_EXTENSION);
            if (file.exists()) {
                RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r");
    ...
    private Page getRightmostLeafPage(File file) throws ExceptionHandler {
        try {
            RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r");
    ...
    private Page getFirstLeafPage(File file) throws ExceptionHandler
    {
        try
        {
            RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r");
    ...
    public int deleteRecord(String databaseName, String tableName, List<InternalCondition> conditions) throws ExceptionHandler
    {
    ...
    private boolean compare(Object object1, Object object2, short condition) throws ExceptionHandler {
        boolean isMatch = false;
            ...
            switch (Utils.resolveClass(object1)) {
                case DatabaseConstants.TINYINT:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.TINYINT:
                            isMatch = ((TinyInt) object1).compare((TinyInt) object2, condition);
                            break;

                        case DatabaseConstants.SMALLINT:
                            isMatch = ((TinyInt) object1).compare((SmallInt) object2, condition);
                            break;

                        case DatabaseConstants.INT:
                            isMatch = ((TinyInt) object1).compare((Int) object2, condition);
                            break;

                        case DatabaseConstants.BIGINT:
                            isMatch = ((TinyInt) object1).compare((BigInt) object2, condition);
                            break;

                        default:
                            throw new ExceptionHandler(ExceptionHandler.DATATYPE_MISMATCH_EXCEPTION, "Number");
                    }
                    break;
                ...
                case DatabaseConstants.SMALLINT:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.TINYINT:
                            isMatch = ((SmallInt) object1).compare((TinyInt) object2, condition);
                            break;

                        case DatabaseConstants.SMALLINT:
                            isMatch = ((SmallInt) object1).compare((SmallInt) object2, condition);
                            break;

                        case DatabaseConstants.INT:
                            isMatch = ((SmallInt) object1).compare((Int) object2, condition);
                            break;

                        case DatabaseConstants.BIGINT:
                            isMatch = ((SmallInt) object1).compare((BigInt) object2, condition);
                            break;
                ...
                case DatabaseConstants.INT:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.TINYINT:
                            isMatch = ((Int) object1).compare((TinyInt) object2, condition);
                            break;

                        case DatabaseConstants.SMALLINT:
                            isMatch = ((Int) object1).compare((SmallInt) object2, condition);
                            break;

                        case DatabaseConstants.INT:
                            isMatch = ((Int) object1).compare((Int) object2, condition);
                            break;

                        case DatabaseConstants.BIGINT:
                            isMatch = ((Int) object1).compare((BigInt) object2, condition);
                            break;
                ...
                case DatabaseConstants.BIGINT:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.TINYINT:
                            isMatch = ((BigInt) object1).compare((TinyInt) object2, condition);
                            break;

                        case DatabaseConstants.SMALLINT:
                            isMatch = ((BigInt) object1).compare((SmallInt) object2, condition);
                            break;

                        case DatabaseConstants.INT:
                            isMatch = ((BigInt) object1).compare((Int) object2, condition);
                            break;

                        case DatabaseConstants.BIGINT:
                            isMatch = ((BigInt) object1).compare((BigInt) object2, condition);
                            break;
                ...
                case DatabaseConstants.REAL:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.REAL:
                            isMatch = ((Real) object1).compare((Real) object2, condition);
                            break;

                        case DatabaseConstants.DOUBLE:
                            isMatch = ((Real) object1).compare((DataType_Double) object2, condition);
                            break;

                        default:
                            throw new ExceptionHandler(ExceptionHandler.DATATYPE_MISMATCH_EXCEPTION, "Decimal Number");
                    }
                    break;
                ...
                case DatabaseConstants.DOUBLE:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.REAL:
                            isMatch = ((DataType_Double) object1).compare((Real) object2, condition);
                            break;
                ...
                case DatabaseConstants.DATE:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.DATE:
                            isMatch = ((DateType) object1).compare((DateType) object2, condition);
                            break;

                        default:
                            throw new ExceptionHandler(ExceptionHandler.DATATYPE_MISMATCH_EXCEPTION, "Date");
                    }
                    break;
                ...
                case DatabaseConstants.DATETIME:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.DATETIME:
                            isMatch = ((DateTime) object1).compare((DateTime) object2, condition);
                            break;

                        default:
                            throw new ExceptionHandler(ExceptionHandler.DATATYPE_MISMATCH_EXCEPTION, "Datetime");
                    }
                    break;
                ...
                case DatabaseConstants.TEXT:
                    switch (Utils.resolveClass(object2)) {
                        case DatabaseConstants.TEXT:
                            if (((Text) object1).getValue() != null) {
                                if (condition != InternalCondition.EQUALS) {
                                    throw new ExceptionHandler(ExceptionHandler.INVALID_CONDITION_EXCEPTION, "= is");
                                } else
                                    isMatch = ((Text) object1).getValue().equalsIgnoreCase(((Text) object2).getValue());
                            }
                            break;

                        default:
                            throw new ExceptionHandler(ExceptionHandler.DATATYPE_MISMATCH_EXCEPTION, "String");
                    }
                    break;
            }
        }
    ...
    private DataRecord readDataRecord(RandomAccessFile randomAccessFile, int pageNumber, short address) throws ExceptionHandler {
        try {
            if (pageNumber >= 0 && address >= 0) {
                ...
                for (byte i = 0; i < numberOfColumns; i++) {
                    switch (serialTypeCodes[i]) {
                        ...
                        case DatabaseConstants.ONE_BYTE_NULL_SERIAL_TYPE_CODE:
                            object = new Text(null);
                            break;

                        case DatabaseConstants.TWO_BYTE_NULL_SERIAL_TYPE_CODE:
                            object = new SmallInt(randomAccessFile.readShort(), true);
                            break;

                        case DatabaseConstants.FOUR_BYTE_NULL_SERIAL_TYPE_CODE:
                            object = new Real(randomAccessFile.readFloat(), true);
                            break;
                        ...
                        case DatabaseConstants.TINY_INT_SERIAL_TYPE_CODE:
                            object = new TinyInt(randomAccessFile.readByte());
                            break;

                        case DatabaseConstants.SMALL_INT_SERIAL_TYPE_CODE:
                            object = new SmallInt(randomAccessFile.readShort());
                            break;

                        case DatabaseConstants.INT_SERIAL_TYPE_CODE:
                            object = new Int(randomAccessFile.readInt());
                            break;

                        case DatabaseConstants.BIG_INT_SERIAL_TYPE_CODE:
                            object = new BigInt(randomAccessFile.readLong());
                            break;

                        case DatabaseConstants.REAL_SERIAL_TYPE_CODE:
                            object = new Real(randomAccessFile.readFloat());
                            break;
                        ...
                        case DatabaseConstants.DATE_SERIAL_TYPE_CODE:
                            object = new DateType(randomAccessFile.readLong());
                            break;

                        case DatabaseConstants.DATE_TIME_SERIAL_TYPE_CODE:
                            object = new DateTime(randomAccessFile.readLong());
                            break;
                        ...
                        default:
                            if (serialTypeCodes[i] > DatabaseConstants.TEXT_SERIAL_TYPE_CODE) {
                                ...
                                object = new Text(new String(text));
                            } else
                                object = null;
                            break;
                    }
